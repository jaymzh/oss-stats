...
def collect_team_data(meeting_date)
  ...
  teams.each do |team|
    ...
    if present
      ...
      fix_points = build_status.include?("red") ? (prompt_yes_no("Did they point to work to fix the build?") ? "Y" : "N") : "-"
...
def generate_plots
  ...
  attendance = Hash.new { |h, k| h[k] = [] }
  build_status = Hash.new { |h, k| h[k] = [] }
  dates = data.map { |row| row[0] }.uniq

  data.each do |date, team, present, build|
    attendance[team] << (present == 'Y' ? 1 : 0)
    build_status[team] << (build == 'N' ? 0 : 1)
  end

  g = Gruff::Line.new
  g.title = "Team Attendance Over Time"
  attendance.each { |team, values| g.data(team, values) }
  g.labels = dates.each_with_index.to_h
  g.write("attendance.png")

  g2 = Gruff::Line.new
  g2.title = "Build Status Reported Over Time"
  build_status.each { |team, values| g2.data(team, values) }
  g2.labels = dates.each_with_index.to_h
  g2.write("build_status.png")
end
