...
def get_pr_and_issue_stats(client, options)
  ...
  pr_stats = { opened: 0, closed: 0, total_close_time: 0.0 }
  issue_stats = { opened: 0, closed: 0, total_close_time: 0.0 }
  page = 1

  loop do
    items = client.issues(repo, state: "all", per_page: 100, page: page)
    ...
    items.each do |item|
      ...
      if options[:verbose]
        puts "Checking item: #{is_pr ? "PR" : "Issue"}, Created at #{created_date}, Closed at #{closed_date || "N/A"}"
      end
      ...
      if closed_date && closed_date >= cutoff_date
        stats[:closed] += 1
        stats[:total_close_time] += (item.closed_at - item.created_at) / 3600.0 # Convert seconds to hours
        all_items_before_cutoff = false
      end
    end
  ...
  pr_stats[:avg_time_to_close_hours] = pr_stats[:closed] == 0 ? 0 : pr_stats[:total_close_time] / pr_stats[:closed]
  issue_stats[:avg_time_to_close_hours] = issue_stats[:closed] == 0 ? 0 : issue_stats[:total_close_time] / issue_stats[:closed]
  ...
  options[:branches].each do |branch|
    ...
    workflows.each do |workflow|
      ...
      workflow_runs.each do |run|
        ...
        jobs.each do |job|
          puts "    Looking at job #{job.name} [#{job.conclusion}]" if options[:verbose]
...
OptionParser.new do |opts|
  ...
  opts.on(
    ...
    unless invalid_modes.empty?
      raise OptionParser::InvalidArgument, "Invalid mode(s): #{invalid_modes.join(", ")}. Valid modes are: #{valid_modes.join(", ")}"
    end
